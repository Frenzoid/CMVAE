"""
Pruning procedure on CUBICC dataset
"""
import os
import sys
import argparse
...
def prune_clusters_and_calculate_metrics_single_step(dl, idxs_to_prune=None):
    ...
    Inputs:
        ...
        idxs_to_prune: list of clusters to prune. None for no clusters annealed
    Outputs:
        nmi over dataset,
        ari over dataset,
        acc over dataset,
        generations for active (i.e. non-pruned) clusters,
        dictionary of probability mass per clusters,
    ...
    with torch.no_grad():
        ...
        for i, dataT in enumerate(dl):
            ...
            for r, qu_x in enumerate(qu_xs):
                ...
                lpc = torch.log(pc + 1e-35) # small constant added for numerical stability
            ...
            # Get cluster assignments given assignments from single modalities
            pc_zs = torch.stack(pc_zs_single_datapoint, dim=-1).squeeze(0)
            _, cluster_assignments_agg, pc_zs_agg = get_cluster_assignments_aggregating_modalities(pc_zs)

            # Update probabiliy mass per cluster
            for val in range(model.params.latent_dim_c):
                prob_mass_per_cluster[val] = prob_mass_per_cluster[val] + ((cluster_assignments_agg == val) * pc_zs_agg).sum().item()

            # Average log-likelihood across modalities
            lliks_single_datapoint_avg = torch.stack(lliks_single_datapoint, dim=0).mean(0)
            # Average penalilized norm across modalities
...
def get_cluster_assignments_aggregating_modalities(pc_zs):
    """
    Aggregate cluster assignments across modalities. Given that is only two modalities and the mode does not make sense, take most confident modality.
    Args:
        pc_zs: Posterior cluster assignments per modality

    Returns:
        selected_modality: most confident modality per sample
        assignments: cluster assignment per sample
        max_confidence: confidence for most confident modality per sample
    """
    pc_zs_flat = pc_zs.view(pc_zs.size(0), -1)
    max_confidence, max_idxs = torch.max(pc_zs_flat, axis=1)
    selected_modality_and_assignments = torch.stack([max_idxs // pc_zs.size(-1), max_idxs % pc_zs.size(-1)], -1)
    selected_modality = selected_modality_and_assignments[:,1]
    assignments = selected_modality_and_assignments[:,0]
    return selected_modality, assignments, max_confidence
