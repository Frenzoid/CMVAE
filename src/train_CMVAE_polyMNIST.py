# Train CMVAE model PolyMNIST dataset
import os
import argparse
import sys
import json
...
from utils import Logger, Timer, save_model_light,unpack_data,  NonLinearLatent_Classifier, get_10_polymnist_samples, ClfImg
import shutil
import torch
...
# Argument to parse
parser = argparse.ArgumentParser(description='CMVAE PolyMNIST Experiment')
parser.add_argument('--experiment', type=str, default='', metavar='E',
                    help='experiment name')
parser.add_argument('--obj', type=str, default='iwae', choices=['iwae', 'dreg'],
...
parser.add_argument('--latent-dim-c', type=int, default=40,
                    help='number of latent clusters')
parser.add_argument('--learn-prior-c', action='store_true', default=True,
                    help='learn prior probabilities for latent clusters')
...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        test_selected_samples = get_10_polymnist_samples(validation_loader.dataset,
                                                     num_testing_images=validation_loader.dataset.__len__(), device=device)
...
def unconditional_coherence():
    ...
    with torch.no_grad():
        ...
        for i, dataT in enumerate(validation_loader):
            ...
            # labels = labels_batch.cpu().data.numpy().reshape(b_size, 10)
            # Joint generation
            uncond_gens = model.generate_unconditional(N=b_size, indexes_to_prune=None, indexes_to_select=None, random=True, coherence_calculation=True, fid_calculation=False)
...
def non_linear_latent_classification(epochs, mod):
    ...
    classifier_u = NonLinearLatent_Classifier(args.latent_dim_z + args.latent_dim_w, 10).to(device)
...
if __name__ == '__main__':
    with Timer('CMVAE') as t:
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 25 == 0:
                # Generate samples
                gen_samples = model.generate_unconditional(N=100, indexes_to_prune=None, indexes_to_select=None, coherence_calculation=False, fid_calculation=False, random=True)
                for j in range(NUM_VAES):
